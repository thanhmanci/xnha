...
- Sau khi cập nhật nội dung xong, chuẩn bị commit và push thay đổi của workspace này lên GitHub nếu môi trường đã có cấu hình Git hợp lệ.
- Remote repo mặc định cho workspace này là: `https://github.com/thanhmanci/xnha.git`
- Quy trình Git ưu tiên dùng:
  - `git remote add origin https://github.com/thanhmanci/xnha.git` nếu chưa có `origin`
  - `git branch -M main`
  - `git add ...`
  - `git commit -m "<message phù hợp>"`
  - `git push -u origin main`
- Nếu thiếu `user.name`, `user.email`, hoặc thiếu quyền xác thực GitHub, phải báo rõ là bị chặn ở bước nào.
